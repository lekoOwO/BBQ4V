...
module.exports = {
    ...
    find: function (req, callback) {
        if (req.query.clipId && req.query.videoId) {
            const sql = mysql.format('SELECT * FROM ' + tableName + ' WHERE videoId = ? AND clipId = ?', [req.query.videoId, req.query.clipId]);
            return db.query(sql, callback);
        } else if (req.query.clipId) {
            const sql = mysql.format('SELECT * FROM ' + tableName + ' WHERE clipId = ?', [req.query.clipId]);
            return db.query(sql, callback);
        } else if (req.query.videoId) {
            const sql = mysql.format('SELECT * FROM ' + tableName + ' WHERE videoId = ?', [req.query.videoId]);
            return db.query(sql, callback);
        } else {
            const sql = mysql.format('SELECT NULL');
            return db.query(sql, callback);
        }
    }
};
